...
if __name__ == "__main__":
    ...
    for config in individual_configs:
        ...
        if os.path.exists(config.output_dir) and not any(
            ...
            command = f"""sbatch --partition=mcml-hgx-a100-80x4 --ntasks=1 --gres=gpu:1 --time=0-10:00:00 --qos=mcml --job-name=benchmark_experiment_sst-5_eval_all --output=logs/%x-%j.out --error=logs/%x-%j.err --wrap "poetry run python scripts/evaluate_prompts.py --experiment-path {complete_path[2].replace("\\", "/")}/\""""
