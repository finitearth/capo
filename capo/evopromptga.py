...
class EvoPromptGA(BaseOptimizer):
    ...
    def optimize(self, n_steps: int) -> List[str]:
        ...
        self.prompts = [
            prompt for _, prompt in sorted(zip(self.scores, self.prompts), reverse=True)
        ]
        ...
        for _ in range(n_steps):
            ...
            self.prompts = [prompt for _, prompt in sorted(zip(scores, prompts), reverse=True)][
                : len(self.prompts)
            ]
    ...
    def _crossover(self, prompts, scores) -> str:
        ...
        for _ in self.prompts:
            ...
            meta_prompt = self.prompt_template.replace("<prompt1>", parent_1).replace(
                "<prompt2>", parent_2
            )
        ...
        if self.verbosity > 1:
            ...
            logger.warning(child_prompts)
        child_prompts = [
            prompt.split("<prompt>")[-1].split("</prompt>")[0].strip() for prompt in child_prompts
        ]
